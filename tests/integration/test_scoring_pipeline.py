...
class TestScoringPipeline:
    ...
    def test_full_scoring_flow_with_firestore(
        ...
        with (
            ...
            # Each face: 95.0 base score × 0.3 size multiplier (1% relative size)
            # detection_confidence = 0.5 (neutral, no bonus)
            # Total: 2 faces × 95.0 × 0.3 = 57.0
            smile_result = calculate_smile_score(test_image_bytes)
            assert smile_result["smile_score"] == 57.0  # 2 faces × 95.0 × 0.3
        ...
        assert doc_data["smile_score"] == 57.0  # 2 faces × 95.0 × 0.3 (1% face size)
        assert doc_data["ai_score"] == 85
        assert doc_data["total_score"] == 48.45  # 57.0 × 85 / 100
        ...
        assert updated_user["best_score"] == 48.45
