...
def show_stats(event_id: str):
    ...
    # Completed images
    completed_images = [
        img for img in images if img.to_dict().get("status") == "completed"
    ]
    ...
    if completed_images:
        ...
        top_images = sorted(
            completed_images,
            key=lambda x: x.to_dict().get("total_score", 0),
            reverse=True,
        )[:3]
        ...
        for i, img_doc in enumerate(top_images, 1):
            ...
            user_name = (
                user_doc.to_dict().get("name", "Unknown")
                if user_doc.exists
                else "Unknown"
            )
            ...
            medal = ["🥇", "🥈", "🥉"][i - 1]
            print(
                f"   {medal} {i}位: {img_data.get('total_score', 0):.2f}点 - {user_name}"
            )
            print(
                f"      (笑顔: {img_data.get('smile_score', 0):.1f}, AI: {img_data.get('ai_score', 0)})"
            )
